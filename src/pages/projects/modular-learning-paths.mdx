...
commercetools serves a wide range of audiences, from technical developers building API integrations to business administrators configuring commerce workflows. Each persona has different learning needs, time constraints, and success criteria. Without role-appropriate learning pathways, users struggled to find relevant content. That meant longer onboarding times, more support dependency, and delayed time-to-value.
...
Create a modular learning ecosystem that serves multiple personas through role-appropriate pathways, while keeping the knowledge interconnected so it teaches systems thinking, not isolated skills.
...
- **Track prerequisites across paths:** Some advanced modules require knowledge from different learning paths. How do we keep learners from getting stuck?
...
Content difficulty scales systematically within each path. Learners start with foundational concepts (e.g., "Organizations and Projects") before tackling advanced patterns (e.g., "Custom checkout implementation"). This supports zone of proximal development, where learners are challenged but not overwhelmed.
...
Learning Path (9 paths) → Module (30+ modules) → Page (200+ pages). This hierarchy provides natural scaffolding. Each tier is a cognitive "chunk" of knowledge, which keeps a large body of content manageable.
...
Worked with the VP of Customer Enablement to define learning path priorities based on business impact. Aligned the curriculum roadmap with product launches, certification requirements, and customer onboarding needs, so learning investments supported strategic objectives.
...
- **Explicit prerequisites reduce frustration.** Declaring required foundational knowledge stops learners from attempting advanced modules before they're ready. This respects zone of proximal development, where content should challenge but not overwhelm.
...
- **Standardization reduces cognitive load.** A consistent module structure makes the learning environment predictable. Learners can focus on content mastery rather than figuring out how to get around.

- **Metadata enables personalization.** Rich configuration (duration, audience, prerequisites) supports filtering, recommendation, and progress tracking. That's the foundation for future AI-powered adaptive learning.
