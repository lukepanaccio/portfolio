...
description: 'A three-agent system (built on Copilot Skills) that detects, maps, and drafts updates for documentation impacted by 250+ API releases per year—catching drift before it reaches developers.'
...
**1. Augment, Don't Replace:** Identify irreducible human decision points and design around them. The system narrows the reviewer's job from "read every release against every module" to "approve or reject scoped, pre-drafted updates"—reducing decision volume rather than eliminating judgment where context matters.
